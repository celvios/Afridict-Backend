...
Status: phases 1-5 have engineering foundations and a synthetic frontend demo; specialist approvals and production integration remain open.
Updated: 13 September 2026. Fourteen total phases; nine remain. Completed phase numbers are historical and are not renumbered.
...
| 6. Contact identity and capabilities | OIDC-compatible registration contract; email/phone verification behind Twilio ports; SendGrid events; normalized Persona identity; action-based capability policy, audit and abuse controls | Registration, email/phone code flow, identity status, `GET /v1/me/capabilities` and blocked-action reasons | Verified contact journey; duplicate/timeout/out-of-order tests; NGN withdrawal denied without verified identity; no provider secret or raw KYC in browser/API |
...
| 12. Developer, partner and data products | API keys/OAuth, scopes, quotas, metering/billing, signed webhooks, sandbox, history/probability quality, widgets, TS/Python/Go SDKs and ABI packages | Developer portal, usage/log views, sandbox and partner examples | SDK/webhook compatibility; replay protection; environment isolation; quotas; history reconciles with canonical events |
...
- Phase 3 engineering outputs: OIDC verification boundary, fail-closed configuration, self-service account creation, server-owned roles, session/eligibility reads and two-actor compliance decisions. Production identity provider, MFA/recovery and privileged account provisioning require selected vendors and human controls.
...
- Next engineering sequence: complete contact identity and capability boundaries; validate OIDC password ownership and provider contracts; implement NGN funding under separate ledger authority; then advance trading, resolution, chain, developer access and launch assurance.
