...
interface ClobMarket {market_id:string;asset_code:string;asset_scale?:number;contract_unit_minor:string;
...
export async function activateClob(sql:Sql,actor:Account,marketId:string,request:string) {
  ...
  const binding=(await sql.query<{asset_code:string;scale:number;contract_unit_minor:string;approved:boolean;synthetic:boolean;asset_approved:boolean}>(`SELECT b.asset_code,
    b.contract_unit_minor::text,b.approved,a.scale,a.synthetic,a.approved AS asset_approved FROM clob_asset_bindings b
    JOIN financial_assets a ON a.code=b.asset_code WHERE b.policy_ref=$1 FOR SHARE`,[market.terms.risk.settlement_asset_ref])).rows[0];
  ...
  await sql.query(`INSERT INTO clob_markets(market_id,asset_code,contract_unit_minor,status,activated_by)
    VALUES ($1,$2,$3,'halted',$4) ON CONFLICT DO NOTHING`,[marketId,binding.asset_code,binding.contract_unit_minor,actor.id]);
...
export async function haltClob(sql:Sql,actor:Account,marketId:string,request:string) {
  const book=(await sql.query<ClobMarket>(`SELECT m.*,a.scale AS asset_scale FROM clob_markets m
    JOIN financial_assets a ON a.code=m.asset_code WHERE m.market_id=$1 FOR UPDATE OF m`,[marketId])).rows[0];
...
export async function orderBook(sql:Sql,marketId:string,outcomeId:string) {
  ...
  const book=(await sql.query<ClobMarket>(`SELECT m.*,a.scale AS asset_scale FROM clob_markets m
    JOIN financial_assets a ON a.code=m.asset_code WHERE m.market_id=$1 FOR SHARE OF m`,[marketId])).rows[0];
...
export async function marketCollateralPolicy(sql:Sql,marketId:string){
  const market=await getMarket(sql,marketId);requireCondition(market.published_at,404,'NOT_FOUND','Market not found.');
  const row=(await sql.query<{asset_code:string;asset_scale:number;contract_unit_minor:string;status:string}>(`SELECT b.asset_code,
    a.scale AS asset_scale,b.contract_unit_minor::text,COALESCE(m.status,'halted') AS status FROM clob_asset_bindings b
    JOIN financial_assets a ON a.code=b.asset_code LEFT JOIN clob_markets m ON m.market_id=$2
    WHERE b.policy_ref=$1 AND b.approved=true AND a.approved=true`,[market.terms.risk.settlement_asset_ref,marketId])).rows[0];
...
export async function marketCollateral(sql:Sql,ownerId:string,marketId:string){
  const row=await marketCollateralPolicy(sql,marketId);
...
export async function listOrders(sql:Sql,ownerId:string,marketId:string) {
  const book=(await sql.query<ClobMarket>('SELECT * FROM clob_markets WHERE market_id=$1',[marketId])).rows[0];
  ...
  return {items:(await sql.query<OrderRow>(`SELECT * FROM clob_orders WHERE owner_id=$1 AND market_id=$2
    ORDER BY sequence DESC LIMIT 100`,[ownerId,marketId])).rows.map(row=>publicOrder(row,book))};
}
...
export async function listFills(sql:Sql,ownerId:string,marketId:string) {
  ...
  return {items:(await sql.query<FillRow>(`SELECT f.* FROM clob_fills f
    ...
    ORDER BY f.sequence DESC LIMIT 100`,[marketId,ownerId])).rows.map(row=>publicFill(row,book))};
}
