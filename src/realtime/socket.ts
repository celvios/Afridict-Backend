...
type Subscription={marketId:string;after:string;outcomes:string[]};
type ClientMessage={type?:unknown;ticket?:unknown;market_id?:unknown;after?:unknown;outcomes?:unknown};
...
export function registerRealtimeSocket(app:FastifyInstance,db:Database,options:RealtimeOptions={}) {
  ...
  app.get('/v1/realtime',{websocket:true,schema:{hide:true}},(socket:WebSocket)=>{
    ...
    const snapshots=async()=>{
      ...
      send(socket,{type:'position_snapshot',market_id:subscription!.marketId,sequence:subscription!.after,
        ...(await listPositions(db,accountId!,subscription!.marketId))});
    };
    const publish=async()=>{
      ...
      working=true;
      try{
        const page=await marketEvents(db,subscription.marketId,subscription.after);
        if(page.items.length){
          for(const event of page.items)send(socket,{type:'market_event',market_id:subscription.marketId,event});
    ...
    socket.on('message',(raw:unknown)=>{
      void (async()=>{
        ...
        try{
          ...
          const initial=await marketEvents(db,message.market_id,message.after);
          subscription={marketId:message.market_id,after:message.after,outcomes};
          send(socket,{type:'subscribed',market_id:subscription.marketId,after:subscription.after,outcomes});
