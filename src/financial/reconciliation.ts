...
export interface ReconciliationSnapshot {
  ...
  finalized_deposits_minor: string; user_claims_minor: string; exceptions: string[]; created_at: string;
}
...
export async function reconcile(sql: Sql, asset: string, actor: string, requestId: string): Promise<ReconciliationSnapshot> {
  ...
  const totals=(await sql.query<{escrow:string;claims:string;chain_deposits:string;chain_withdrawals:string;partner:string;posted:string;
  ...
  if (BigInt(totals.claims)!==BigInt(totals.escrow)) breaks.push('CLAIMS_COLLATERAL_MISMATCH');
  ...
  await sql.query(`INSERT INTO reconciliation_runs
    (id,requested_by,asset_code,status,escrow_ledger_minor,chain_net_minor,partner_deposits_minor,finalized_deposits_minor,user_claims_minor)
    VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,[id,actor,asset,breaks.length?'exceptions_opened':'balanced',totals.escrow,
    chainNet,totals.partner,totals.posted,totals.claims]);
  const snapshot:ReconciliationSnapshot={id,asset,status:breaks.length?'exceptions_opened':'balanced',
    ...
    finalized_deposits_minor:totals.posted,user_claims_minor:totals.claims,exceptions:breaks,created_at:new Date().toISOString()};
