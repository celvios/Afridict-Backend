...
export async function createIdentitySession(sql:Sql,provider:IdentityVerificationProvider,input:{accountId:string;idempotencyKey:string;requestId:string}) {
  const assurance=(await sql.query<{identity_status:string;email_verified_at:Date|null;phone_verified_at:Date|null}>(
    'SELECT identity_status,email_verified_at,phone_verified_at FROM account_assurance WHERE account_id=$1 FOR UPDATE',[input.accountId])).rows[0];
  requireCondition(assurance,409,'REGISTRATION_PROFILE_REQUIRED','Complete registration before identity verification.');
  requireCondition(assurance.email_verified_at&&assurance.phone_verified_at,403,'CONTACT_VERIFICATION_REQUIRED','Verify email and phone before identity verification.');
  ...
  const inquiry=(await sql.query<{id:string;account_id:string}>(`INSERT INTO identity_inquiries(id,account_id,provider,provider_reference,state)
    VALUES ($1,$2,'persona',$3,'PENDING') ON CONFLICT (provider,provider_reference) DO UPDATE SET updated_at=now() RETURNING id,account_id`,
    [randomUUID(),input.accountId,session.providerReference])).rows[0]!;
  requireCondition(inquiry.account_id===input.accountId,409,'IDENTITY_SESSION_CONFLICT','The provider inquiry is already assigned to another account.');
...
export async function applyPersonaEvent(sql:Sql,event:PersonaEvent,raw:Buffer,requestId:string) {
  ...
  const occurred=new Date(event.occurredAt);requireCondition(!Number.isNaN(occurred.valueOf())&&occurred.valueOf()<=Date.now()+300_000,
    400,'MALFORMED_PROVIDER_EVENT','Invalid provider event time.');
