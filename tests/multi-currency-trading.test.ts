...
let db:Database,app:FastifyInstance,trader:string,finance:string,ngnMarket:string,usdtMarket:string;
let key=0;
...
beforeAll(async()=>{
  ...
  await db.query("UPDATE eligibility SET status='eligible',policy_version='synthetic:eligible' WHERE account_id=$1",[trader]);
...
describe('multi-currency market collateral',()=>{
  ...
  it('uses USDT token units without accepting an asset from the activation request',async()=>{
